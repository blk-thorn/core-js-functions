...
function memoize(func) {
  ...
  return function getResult(...args) {
    if (!hasCached || JSON.stringify(args) !== JSON.stringify(cachedArgs)) {
...
function retry(func, attempts) {
  return function returnError(...args) {
    let lastError;
...
function logger(func, logFunc) {
  return function loggedFun(...args) {
    const argsString = args
      .map((arg) => (typeof arg === 'object' ? JSON.stringify(arg) : arg))
      .join(',');
    const logStart = `${func.name}(${argsString}) starts`;
    logFunc(logStart);
    try {
      const result = func.apply(this, args);
      const logEnd = `${func.name}(${argsString}) ends`;
      logFunc(logEnd);
      return result;
    } catch (error) {
      const logEnd = `${func.name}(${argsString}) ends`;
...
function partialUsingArguments(fn, ...args1) {
  return function partArgs(...args2) {
